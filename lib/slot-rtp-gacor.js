...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        'slot-rtp-gacor:toggle-focus-of-active-pane': () => this.toggleFocus(),
        'slot-rtp-gacor:toggle-follow-mode': () => this.toggleFollow()
      })
    )

    if (atom.config.get('slot-rtp-gacor.followModeByDefault')) {
      this.toggleFollow()
    }
  },
  ...
  toggleFollow() {
    ...
    const targets = atom.config.get('slot-rtp-gacor.followLocations')
  ...
  startFollow() {
    const delay = atom.config.get('slot-rtp-gacor.focusDelay')
  ...
  doFocus() {
    ...
    activePane.element && activePane.element.classList.add('slot-rtp-gacor-focus')

    const expandedWidth = atom.config.get('slot-rtp-gacor.expandedWidth') / 100
  ...
  restorePanes() {
    this.modifiedPanes.forEach(({ pane, flexScale }) => {
      ...
      pane.element && pane.element.classList.remove('slot-rtp-gacor-focus')
  ...
  checkIncompatibility(plugin) {
    if (this.incompatiblePlugins.includes(plugin.name)) {
      atom.notifications.addError('Incompatible Package Detected', {
        dismissable: true,
        detail: `slot-rtp-gacor does not work when package "${
          plugin.name
        }" is activated.`
      })
    }
